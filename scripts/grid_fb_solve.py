...
def _load_human(work):
    """upper-body ratings are a top-level Zenodo file; full-body ratings live inside
    genea_2022_subjective_evaluation.zip. Handle both."""
    import json as _json
    hj = os.path.join(work, "hl.json")
    if TIER == "upper-body":
        _zfile(RATINGS_REC, f"{TIER}_human-likeness.json", hj)
        return _json.load(open(hj))
    sz = os.path.join(work, "subj.zip")
    _zfile(RATINGS_REC, "genea_2022_subjective_evaluation.zip", sz)
    with zipfile.ZipFile(sz) as z:
        return _json.loads(z.read(f"{TIER}_human-likeness.json"))
...
def solve(config):
    ...
    try:
        report["human"] = _human_scores(_load_human(work), systems)
    except Exception as e:
